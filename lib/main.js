...
class Provider {
  ...
  loadSuggestions() {
    ...
    try {
      ...
      for (let [idc, mkeys] of Object.entries(keywords)) {
        ...
        for (let [idk, params] of Object.entries(mkeys)) {
          this.suggestions.push({
            idc: idc,
            text: fmt(idk),
            type: "keyword",
          ...
          if (params && typeof params === "object") {
            ...
            for (let [paramName, enumValues] of Object.entries(params)) {
              if (Array.isArray(enumValues)) {
                for (let enumVal of enumValues) {
                  this.suggestions.push({
                    idc: idc,
                    idk: idk,
                    idp: paramName,
  ...
  findParam(editor, bufferPosition) {
    ...
    try {
      ...
      const lineText = editor.getTextInRange([
      ...
      let params = this.keywordsService.getCommandParams(
        moduleName,
        commandName
      );
      const basicParams = this.keywordsService.getCommandParams(
        "BASIC",
        commandName
      );
